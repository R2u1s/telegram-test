      ...
      <div className={styles["theme"]}>
        <div className={styles["theme__cover"]} >
          ...
          <div className={styles["theme__cover-flex"]}>
            ...
            <svg width="60" height="60" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
              <rect width="60" height="60" rx="30" fill="#818181" fillOpacity="0.4" />
        ...
        <ul className={styles["theme__list"]}>
          {mockCourses[0].chapters[0].tracks.map((item, index) => {
            return <ThemeElement key={item.id} id={item.id} index={index + 1} name={item.name} duration={item.duration} done={item.done} callBack={onClick} />
          })}
        </ul>
      </div>
    </>
